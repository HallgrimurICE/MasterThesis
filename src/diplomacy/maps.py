...
def cooperative_attack_initial_state() -> GameState:
    board = square_board()
    units = {
        "A": Unit(Power("Aurora"), "A", UnitType.ARMY),
        "B": Unit(Power("Borealis"), "B", UnitType.ARMY),
        "C": Unit(Power("Crimson"), "C", UnitType.ARMY),
    }
...
def mesh_board_5x3() -> Dict[str, Province]:
    ...
    for r in range(3):
        for c in range(5):
            ...
            if name in {"3", "4", "9", "10", "13", "14"}:
                province_type = ProvinceType.SEA
            elif name in {"1", "2", "5", "11", "15"}:
    ...
    for r in range(3):
        for c in range(5):
            ...
            board[src] = Province(
                ...
                province_type=current.province_type,
            )
    return board
...
def demo_state_mesh() -> GameState:
    ...
    units = {
        "1": Unit(Power("Blue"), "1", UnitType.ARMY),
        "5": Unit(Power("Pink"), "5", UnitType.ARMY),
        "8": Unit(Power("Red"), "8", UnitType.ARMY),
        "11": Unit(Power("Green"), "11", UnitType.ARMY),
        "15": Unit(Power("Yellow"), "15", UnitType.ARMY),
    }
    powers = {u.power for u in units.values()}
    return GameState(board=board, units=units, powers=powers)


def fleet_coast_demo_state() -> GameState:
    board: Dict[str, Province] = {
        "Highlands": Province(
            name="Highlands",
            neighbors={"AlbionBay", "Cliffhaven", "RedKeep", "AzureSea"},
            is_supply_center=True,
            home_power=Power("Blue"),
            province_type=ProvinceType.LAND,
        ),
        "AlbionBay": Province(
            name="AlbionBay",
            neighbors={"Highlands", "Cliffhaven", "AzureSea"},
            is_supply_center=True,
            home_power=Power("Blue"),
            province_type=ProvinceType.COAST,
        ),
        "Cliffhaven": Province(
            name="Cliffhaven",
            neighbors={"Highlands", "AlbionBay", "AzureSea", "RedKeep", "OpenOcean"},
            is_supply_center=True,
            home_power=None,
            province_type=ProvinceType.COAST,
        ),
        "AzureSea": Province(
            name="AzureSea",
            neighbors={"AlbionBay", "Cliffhaven", "OpenOcean", "Highlands", "RedKeep"},
        ...
        "RedKeep": Province(
            name="RedKeep",
            neighbors={"Highlands", "Cliffhaven", "AzureSea", "OpenOcean"},
            is_supply_center=True,
            home_power=Power("Red"),
            province_type=ProvinceType.LAND,
        ),
        "OpenOcean": Province(
            name="OpenOcean",
            neighbors={"Cliffhaven", "AzureSea", "RedKeep"},
            is_supply_center=False,
            home_power=None,
            province_type=ProvinceType.SEA,
        ),
    ...
    units = {
        "Highlands": Unit(Power("Blue"), "Highlands", UnitType.ARMY),
        "AlbionBay": Unit(Power("Blue"), "AlbionBay", UnitType.FLEET),
        "RedKeep": Unit(Power("Red"), "RedKeep", UnitType.ARMY),
        "OpenOcean": Unit(Power("Red"), "OpenOcean", UnitType.FLEET),
    }
    powers = {u.power for u in units.values()}
    return GameState(board=board, units=units, powers=powers)
