...
support-capable sampled best response (SBR).  The focus is on constructing
bundled order candidates that include both the primary move/hold for each
unit *and* plausible support orders from nearby friendly units.  Bundles are
evaluated via a sampled best-response procedure against opponent joint-order
profiles drawn from a stochastic base policy.

The implementation intentionally keeps the heuristics simple:

* Moves are prioritised when they target supply centres, enemy-occupied
  provinces, or locations bordering enemy units.
* Hold actions are retained (optionally) so threatened supply centres can be
  protected with support-hold orders.
* Supporters are ranked via a cheap centrality/frontier heuristic derived from
  their proximity to contested supply centres.

Only the Python standard library and the light-weight data structures from the
``diplomacy`` package are used so the agent can be embedded easily inside
experiments without additional dependencies.
"""
...
from diplomacy.adjudication import Adjudicator
from diplomacy.orders import hold, move, support_hold, support_move
from diplomacy.state import GameState
from diplomacy.types import Power, Unit
...
def propose_bundles(
    ...
    *,
    ...
    Bundles are generated by first choosing a primary move/hold for each unit
    (top-``k_moves`` candidates ranked by :func:`simple_move_score`).  For each
    primary order we attach up to ``m_supports`` supporting orders from nearby
    friendly units where those orders are legal according to the adjudicator.
    Duplicate bundles are removed and the resulting list is filtered so every
    bundle issues exactly one legal order for each unit owned by ``player``.
    """
    ...
    for unit in friendly_units:
        ...
        selected_moves = _select_top(move_candidates, max(0, k_moves - (1 if include_holds else 0)))
            ...
            if parsed.kind == "MOVE" and parsed.target is not None:
                ...
                for supporter in neighboring_friendly_units(state, player, parsed.target):
                    ...
                    if support_order not in legal_map.get(supporter, []):
                        continue
                    supporters.append(
                        (supporter, support_order, simple_support_score(state, supporter, parsed.target))
                    )
                if supporters:
        ...
        for supporters in support_map.values():
            ...
            for variant in bundle_variants:
                ...
                for supporter, support_order, _ in supporters:
                    ...
                    if current_order and current_order != support_order and _parse_order(current_order).kind.startswith("SUPPORT"):
                        conflict = True
                        break
